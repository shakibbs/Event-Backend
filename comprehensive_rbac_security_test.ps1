...
# ============================================================================
# PHASE 1: TEST SUPERADMIN LOGIN & USER CREATION
# ============================================================================
...
if ($superadminToken) {
    ...
    try {
        ...
        if ($response.StatusCode -eq 200 -or $response.StatusCode -eq 201) {
            ...
            $global:report += "  Email: testadmin@example.com"
    ...
    try {
        ...
        if ($response.StatusCode -eq 200 -or $response.StatusCode -eq 201) {
            ...
            $global:report += "  Email: testattendee@example.com"
    ...
    # Test 2.3: Try to create duplicate user (should fail)
    LogSubSection "Test 2.3: Create Duplicate User (Should Fail)"
    try {
        $response = Invoke-WebRequest -Uri "$baseUrl/api/users" `
            -Method Post `
            -Headers @{
            ...
            -Body $adminUserJson `
            -ErrorAction Stop
        LogFail "Duplicate user was created (Validation issue!)"
    } catch {
        if ($_.Exception.Response.StatusCode -eq 400 -or $_.Exception.Response.StatusCode -eq 409) {
            LogPass "Duplicate user correctly rejected (HTTP $($_.Exception.Response.StatusCode))"
        } else {
            LogFail "Unexpected response for duplicate: $($_.Exception.Response.StatusCode)"
        }
    }
    
    # Test 2.4: Try to create user without authentication
    LogSubSection "Test 2.4: Create User Without Authentication"
    $newUserJson = @{
...
# ============================================================================
# PHASE 7: TEST TOKEN REFRESH & LOGOUT
# ============================================================================
